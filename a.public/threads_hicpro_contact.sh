...
{ 
	...
	LANG=en sort --parallel=$thread -k2,2V -k3,3n -k5,5V -k6,6n $wdir/tmp/$key.validPairs > $wdir/$key.sorted.pairs && awk -F"\t" 'BEGIN{c1=0;c2=0;s1=0;s2=0} (c1!=$2||c2!=$5||s1!=$3||s2!=$6){print;c1=$2;c2=$5;s1=$3;s2=$6}' $wdir/$key.sorted.pairs > $wdir/${key}.rmdup.pairs && echo "$key sort & remove PCR duplicate done." || echo "$key sort & remove PCR duplicate error." #-m wrong
	...
	for res in ${ress}; do 
		(cat $wdir/${key}.rmdup.pairs | $HiCPro/build_matrix --binsize $res --chrsizes $chromSize --ifile /dev/stdin --oprefix $wdir/${key}.${res} --matrix-format upper --progress 2> $wdir/logs/6.$key.$res.matrix.log && echo "$key build matrix $res done." || echo "$key build matrix $res error.";
		
		python $HiCPro/ice --results_filename $wdir/${key}.${res}.iced --filter_low_counts_perc $filter_low_counts_perc --filter_high_counts_perc $filter_high_counts_perc --max_iter $max_iter --eps $eps --remove-all-zeros-loci $remove_all_zeros_loci --output-bias $output_bias --verbose $verbose $wdir/${key}.${res}.matrix > $wdir/logs/6.$key.$res.ice.log && echo "$key ICE normalize $res done." || echo "$key ICE normalize $res error.";)&
	done;wait
